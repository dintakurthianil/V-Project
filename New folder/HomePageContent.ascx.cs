...
namespace VrtxIntPortal.HomePageContent
{
    [ToolboxItemAttribute(false)]
    public partial class HomePageContent : WebPart
    {
        ...
        private void GetWhatsNewListItems(string listName, string country)
        {
            ...
            if (WhatsList != null)
            {
                ...
                {
                    whatsNew.InnerHtml = "No Events available";
                }
            }
        }
        /// <summary>
        ...
        private SPListItemCollection GetHolidayListCollection(SPList list, string country)
        {
            ...
            string stringQuery = string.Format(@"<Where><And><And><And><Geq><FieldRef Name='HolidayDate' /><Value Type='DateTime'>{0}</Value></Geq><Leq><FieldRef Name='HolidayDate' /><Value Type='DateTime'>{1}</Value></Leq></And><Eq><FieldRef Name='Country' /><Value Type='Choice'>{2}</Value></Eq></And><Eq><FieldRef Name='FiscalYear' /><Value Type='Lookup'>{3}</Value></Eq></And></Where><OrderBy><FieldRef Name='HolidayDate' Ascending='True' /></OrderBy>", SPUtility.CreateISO8601DateTimeFromSystemDateTime(firstDay), SPUtility.CreateISO8601DateTimeFromSystemDateTime(firstDay.AddMonths(1)), country, DateTime.Now.Year);
          //string stringQuery = string.Format(@"<Where><And><And><And><Geq><FieldRef Name='HolidayDate' /><Value Type='DateTime'>{0}</Value></Geq><Leq><FieldRef Name='HolidayDate' /><Value Type='DateTime'>{1}</Value></Leq></And><Eq><FieldRef Name='Country' /><Value Type='Choice'>{2}</Value></Eq></And><Eq><FieldRef Name='FiscalYear' /><Value Type='Lookup'>{3}</Value></Eq></And></Where><OrderBy><FieldRef Name='HolidayDate' Ascending='True' /></OrderBy>", SPUtility.CreateISO8601DateTimeFromSystemDateTime(firstDay), SPUtility.CreateISO8601DateTimeFromSystemDateTime(firstDay.AddMonths(1)), country, DateTime.Now.Year);
        ...
        private SPListItemCollection GetWhatsNewCollection(SPList list, string country)
        {
            ...
            string stringQuery = string.Format(@"<Where><And><Eq><FieldRef Name='Country' /><Value Type='Choice'>{0}</Value></Eq><And><Eq><FieldRef Name='FiscalYear' /><Value Type='Lookup'>" + date + "</Value></Eq><Eq><FieldRef Name='ShowOnHomePage' /><Value Type='bit'>{1}</Value></Eq></And></And></Where>", country, 1);
            qry.Query = stringQuery;
            //qry.Query = @"<Where><Eq><FieldRef Name='Country' /><Value Type='Choice'>" + country + "</Value></Eq></Where>";
            ...
            using (SPSite site = new SPSite(SPContext.Current.Web.Url))
            {
                ...
                using (SPWeb web = site.OpenWeb())
                {
                    ...
                    if (list != null)
                    {
                        ...
                        {
                            Holidays.InnerHtml = "<img src='/_layouts/15/VrtxIntranetStyles/Images/no-holidays.jpg' />";
                        }
        ...
        private SPListItemCollection GetMessagefromCE0ItemCollection(SPList list)
        {
           ...
           // qry.ViewFields = @"<FieldRef Name='Title' /><FieldRef Name='Message' /><FieldRef Name='Attachments' />";
            ...
            qry.Query = @"<Where><Eq><FieldRef Name='FiscalYear' /><Value Type='Lookup'>" + value + "</Value></Eq></Where>";
            //if(qry.c)
            qry.RowLimit = 3;
                ...
                if (MessageListCollection.Count > 0)
                {
                    foreach (SPListItem item in MessageListCollection)
                    {
                        ...
                        if (!string.IsNullOrEmpty(message))
                        {
                            ...
                            {
                                string messageDesc = message.Substring(0, 480) + ".....";
                        ...
                        if (ceocount == 0)
                        {
                            ...
                            {
                                picture1.Src = "/_layouts/15/VrtxIntranetStyles/Images/NoImage.jpg";
                            }
                            if (sbMessage.Length > 0)
                            {
                                content1.InnerHtml = sbMessage.ToString();
                                message1.InnerHtml = title;
                            }
                        ...
                        else if (ceocount == 1)
                        {
                            ...
                            {
                                picture2.Src = "/_layouts/15/VrtxIntranetStyles/Images/NoImage.jpg";
                            }
                            if (sbMessage.Length > 0)
                            {
                                content2.InnerHtml = sbMessage.ToString();
                                message2.InnerHtml = title;
                            }
                            ceocount += 1;
                        }
                        else if (ceocount == 2)
            ...
            if (spBasicInfo != null)
            {
                ...
                if (empCllection != null && empCllection.Count > 0)
                {
                    foreach (SPListItem item in empCllection)
                    {
                        ...
                        if (item.Attachments.Count > 0)
                        {
                            foreach (String attachmentname in item.Attachments)
                            {
                               string  attachmentAbsoluteURL = item.Attachments.UrlPrefix + attachmentname;
                                attachmentUrl = attachmentAbsoluteURL.ToString();
                            }
                        }
                        else
                        {
                            attachmentUrl = "/_layouts/15/VrtxIntranetStyles/Images/NoImage.jpg";
                        }
                        ...
                        empreadmore.HRef = string.Format("/SitePages/ODCServiceProfiles.aspx?username={0}", userName);
